#!/usr/bin/env python3
"""Qwen3.5-2B 使用示例（NVIDIA GPU 移植版）。

在 NVIDIA GPU 机器上运行：

    方式 1 —— 直接跑本脚本（加载引擎 + 真实请求）:
        python examples/usage_qwen35.py [--model-path Qwen/Qwen3.5-2B] [--device cuda]

    方式 2 —— 命令行（引擎入口 qwen35_run）:
    ...
    验收（oracle 对拍，误差 < 0.002 才算移植数值正确）:
        python examples/verify_scoring_torch.py --backend qwen35 \
...
def main():
    parser = argparse.ArgumentParser(description="Qwen3.5-2B 使用示例（NVIDIA GPU）")
    ...
    print(f"加载 Qwen3.5-2B: {args.model_path} (device={args.device}) ...")
    ...
    print("\n提示: 概率是相对给定候选归一化的，非校准。Qwen3.5 模板的思考模式已自动禁用"
          "（打分离不开思考）。")
    print("数值验收请运行: python examples/verify_scoring_torch.py --backend qwen35 "
